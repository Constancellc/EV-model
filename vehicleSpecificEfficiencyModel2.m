function vehicleSpecificEfficiencyModel2()
% This model is an academic exercise. It finds the efficiency value for
% each vehicle and drive cycle combo which gives basically zero error This
% is designed so that we can test models on the efficiencies - obviously
% the model is an example of overfitting!
...
x = fminunc(@likelihood,[0.3;ones(2*N,1)],options)
...
function [f,g] = likelihood(x)
    ...
    for v = 1:N
        ...
        for i = 1:T_u-1
            if a_u(i) < 0
                eff_u(i) = eff(v+N); deff_u(i) = 1;
            else
                eff_u(i) = 1/(eff(v+N)); deff_u(i) = -1/(eff(v+N)^2);
            end
        end
        ...
        g(v+1) = (1/var^2)*(dH*P_h(v,:)*deff_h);
        g(v+1+N) = (1/var^2)*(dU*P_u(v,:)*deff_u);
    end
